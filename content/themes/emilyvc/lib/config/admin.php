...
function mixd_screen_layout_columns($columns) {
    $columns['dashboard'] = 0;
    return $columns;
}
add_filter('screen_layout_columns', 'mixd_screen_layout_columns');

function mixd_screen_layout_dashboard() {
    return 2;
}
add_filter('get_user_option_screen_layout_dashboard', 'mixd_screen_layout_dashboard');

function registerCustomCSS(){
    $src = get_stylesheet_directory_uri()."/assets/css/admin.css";
    $handle = "Custom-Admin";
    wp_register_script($handle, $src);
    wp_enqueue_style($handle, $src, array(), false, false);
}
add_action('admin_head', 'registerCustomCSS');
...
function my_custom_dashboard_widgets() {
    ...
    wp_add_dashboard_widget('dashboard-latest-news', 'Latest News', 'custom_dashboard_latest_news');

}

function custom_dashboard_latest_news() {
        ...
        echo '<tbody id="the-list">';
            while( $news->have_posts() ): $news->the_post();
                $id = get_the_ID();
                $admin_url = get_admin_url('', 'post.php?post='.$id.'&action=edit');
                ...
                echo '<tr class="post-title page-title column-title '.$row_class.'">';
                    // Title
                    echo '<td class="column-title" style="width: 45%">';
                    if(current_user_can('edit_others_posts')):
                        echo '<strong><a href="'.$admin_url.'" class="row-title">';
                        echo get_the_title();
                        echo '</strong></a>';
                    else:
                        echo '<strong class="row-title">';
                        echo get_the_title();
                        echo '</strong>';
                    endif;
                    ...
                    // Read link
                    echo '<td style="width: 52px !important">';
                        echo '<a href="'.get_the_permalink().'" target="_blank">Read</a>';
                    echo '</td>';
                echo '</tr>';
            $inc++; endwhile; wp_reset_postdata();
        echo '</tbody>';
        echo '</table>';
}
